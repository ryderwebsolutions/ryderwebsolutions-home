# Electrical Website Template

This folder contains a static HTML/CSS template designed for a Dublin-based electrical business. It can be hosted locally or deployed to a platform such as Vercel.
...
- Dynamic link and asset rewriting:
  ...
  - Use `data-href="electrician.css"` on `<link>` tags for CSS.
...
> _Tip:_ If you deploy to a subpath (e.g. `https://example.com/electrician/`), the dynamic rewriting will adjust links automatically.
...
- Modify colors by adjusting CSS variables in `electrician.css`.
