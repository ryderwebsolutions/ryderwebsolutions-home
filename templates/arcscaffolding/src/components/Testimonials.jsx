...
const StarRating = () => (
  <div className="flex gap-0.5 mb-3" aria-label="5 stars">
    {[...Array(5)].map((_, i) => (
      <svg key={i} className="w-4 h-4 text-arc-orange" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t) => (
            <div
              key={t.name}
              className="bg-arc-grey border border-gray-200 rounded-xl p-7 hover:shadow-card hover:border-arc-orange/20 transition-all duration-300 flex flex-col"
            >
              {/* Quote mark */}
              <svg
                className="w-10 h-10 text-arc-orange/25 mb-3 flex-shrink-0"
                fill="currentColor"
                viewBox="0 0 32 32"
                aria-hidden="true"
              >
              ...
              {/* Quote */}
              <blockquote className="text-arc-steel text-sm leading-relaxed flex-1 mb-5">
                "{t.quote}"
              </blockquote>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-gray-200">
                ...
                <div>
                  <p className="text-navy font-bold text-sm leading-tight">{t.name}</p>
                  <p className="text-arc-steel text-xs mt-0.5">{t.role}</p>
                </div>
              </div>
            </div>
          ))}
