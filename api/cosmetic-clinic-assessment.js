...
const validateFormData = (data) => {
    const requiredFields = [
        'clinic_name', 'your_name', 'work_email', 'phone_number',
        'clinic_location', 'business_type', 'main_goal', 'biggest_challenge', 'timeline'
    ];
...
const formatEmailBody = (data) => {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; padding: 30px;">
        <h2 style="color: #7b2cbf; border-bottom: 2px solid #9d4edd; padding-bottom: 10px;">
            New Cosmetic Clinic Assessment
        </h2>
        
        <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            <h3 style="color: #333; margin-top: 0;">Contact Information</h3>
            <table style="width: 100%; border-collapse: collapse;">
                <tr><td style="padding: 8px 0; color: #666; width: 40%;">Clinic Name:</td><td style="padding: 8px 0; font-weight: bold;">${escapeHtml(data.clinic_name)}</td></tr>
                ...
                <tr><td style="padding: 8px 0; color: #666;">Location:</td><td style="padding: 8px 0;">${escapeHtml(data.clinic_location)}</td></tr>
            </table>
        </div>
    ...
    try {
        ...
        if (process.env.EMAIL_SERVICE === 'resend') {
            ...
            await resend.emails.send({
                ...
                subject: `New Assessment: ${data.clinic_name} — ${data.business_type}`,
        ...
        console.log(`Assessment submitted: ${data.clinic_name} <${data.work_email}>`);
        return res.status(200).json({ success: true });
